...
import { MAX_CAR_IMAGE_COUNT, MAX_CAR_IMAGE_SIZE_BYTES, RUSSIAN_PLATE_PATTERN, normalizePlateNumberInput } from '@/utils/car-form'
...
<template>
  <div class="grid gap-4 md:grid-cols-2">
    <label class="field-group">
      ...
      <input class="input-base" list="car-makes" :value="modelValue.make" @input="updateTextField('make', ($event.target as HTMLInputElement).value)" />
    ...
    <label class="field-group">
      ...
      <input class="input-base" list="car-models" :value="modelValue.model" @input="updateTextField('model', ($event.target as HTMLInputElement).value)" />
    ...
    <label class="field-group">
      ...
      <select class="input-base" :value="modelValue.location" @change="update('location', ($event.target as HTMLSelectElement).value)">
      ...
      <div class="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          ...
          <div v-if="modelValue.imageUrls.length" class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
            <div v-for="(imageUrl, index) in modelValue.imageUrls" :key="`${imageUrl}-${index}`" class="space-y-2">
              ...
              <button class="btn-secondary w-full" type="button" @click="removeImage(index)">{{ copy.imageRemove }}</button>
            </div>
          </div>
        </div>
        <div class="flex flex-col gap-3">
          ...
          <div class="flex flex-wrap gap-3">
            ...
            <button v-if="modelValue.imageUrls.length" class="btn-secondary" type="button" @click="clearImage">{{ locale === 'ru' ? 'Очистить все' : 'Clear all' }}</button>
          </div>
