...
import {
  ...
  hasBlockingRental,
  getMaintenanceOverlapReason,
  getRentalOverlapReason,
} from '@/utils/business'
...
function restoreCarOperationalStatus(state: MockDatabaseState, carId: string): void {
  ...
  const inProgressMaintenance = state.maintenanceWindows.some(
    (window) =>
      window.carId === carId && window.status === 'IN_PROGRESS' && overlaps(window.from, window.to, today, today),
  )
...
export const mockDb = {
  ...
  async deleteCar(carId: string): Promise<void> {
    const state = readState()
    ensureCarExists(state, carId)
    const hasActiveRental = state.rentals.some(
      (rental) => rental.carId === carId && ['CREATED', 'CONFIRMED', 'ISSUED'].includes(rental.status),
    )
    if (hasActiveRental) {
  ...
  async deleteTariff(tariffId: string): Promise<void> {
    ...
    const hasLinkedRentals = state.rentals.some(
      (rental) => rental.tariffId === tariffId && ['CREATED', 'CONFIRMED', 'ISSUED'].includes(rental.status),
    )
    if (hasLinkedRentals) {
  ...
  async createMaintenance(payload: Omit<MaintenanceWindow, 'id' | 'car'>): Promise<MaintenanceWindow> {
    ...
    const blockingMaintenance = hasBlockingMaintenance(
  ...
  async updateMaintenance(
    windowId: string,
    payload: Omit<MaintenanceWindow, 'id' | 'car'>,
  ...
  async getDashboardStats(): Promise<DashboardStats> {
    ...
    const stats: DashboardStats = {
      ...
      plannedMaintenance: state.maintenanceWindows.filter((window) =>
        ['SCHEDULED', 'IN_PROGRESS'].includes(window.status),
      ).length,
