...
export const carsApi = {
  ...
  getById: async (id: string) =>
    USE_MOCK_API ? mockDb.getCar(id) : mapBackendCar(await http.get<BackendCar>(`/cars/${id}`)),
  ...
  searchAvailable: (params: SearchCarsParams) => {
    ...
    return http
      .get<BackendAvailableCarResult[]>(`/cars/available?${query.toString()}`)
      .then((items) => items.map(mapBackendAvailability))
  },
}
