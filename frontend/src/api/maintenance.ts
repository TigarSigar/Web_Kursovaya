...
export const maintenanceApi = {
  list: async () =>
    USE_MOCK_API
      ? mockDb.listMaintenance()
      : (await http.get<BackendMaintenanceWindow[]>('/maintenance-windows')).map(mapBackendMaintenanceWindow),
  getById: (id: string) =>
    USE_MOCK_API
      ? mockDb.getMaintenance(id)
      : http.get<BackendMaintenanceWindow>(`/maintenance-windows/${id}`).then(mapBackendMaintenanceWindow),
  create: (payload: Omit<MaintenanceWindow, 'id' | 'car'>) =>
    USE_MOCK_API
      ? mockDb.createMaintenance(payload)
      : http
          .post<BackendMaintenanceWindow>('/maintenance-windows', toBackendMaintenancePayload(payload))
          .then(mapBackendMaintenanceWindow),
  update: (id: string, payload: Omit<MaintenanceWindow, 'id' | 'car'>) =>
    USE_MOCK_API
      ? mockDb.updateMaintenance(id, payload)
      : http
          .put<BackendMaintenanceWindow>(`/maintenance-windows/${id}`, toBackendMaintenancePayload(payload))
          .then(mapBackendMaintenanceWindow),
  remove: (id: string) =>
