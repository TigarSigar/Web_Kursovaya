...
export const tariffsApi = {
  list: async () =>
    USE_MOCK_API ? mockDb.listTariffs() : (await http.get<BackendTariff[]>('/tariffs')).map(mapBackendTariff),
  getById: async (id: string) =>
    USE_MOCK_API ? mockDb.getTariff(id) : mapBackendTariff(await http.get<BackendTariff>(`/tariffs/${id}`)),
  ...
  update: (id: string, payload: Omit<Tariff, 'id'>) =>
    USE_MOCK_API
      ? mockDb.updateTariff(id, payload)
      : http.put<BackendTariff>(`/tariffs/${id}`, payload).then(mapBackendTariff),
