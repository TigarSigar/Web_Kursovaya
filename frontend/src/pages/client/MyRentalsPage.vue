...
const copy = computed(() =>
  locale.value === 'ru'
    ...
    : {
        ...
        cancelText:
          'Cancellation is allowed only before the vehicle is issued. Once the rental reaches ISSUED, this action is no longer available.',
...
<template>
  <section>
    ...
    <div class="mb-6 flex flex-wrap gap-2">
      <button class="chip" :class="{ 'chip-active': activeTab === 'all' }" type="button" @click="activeTab = 'all'">
        {{ t('common.all') }}
      </button>
      <button
        class="chip"
        :class="{ 'chip-active': activeTab === 'active' }"
        type="button"
      ...
      <button
        class="chip"
        :class="{ 'chip-active': activeTab === 'history' }"
        type="button"
    ...
    <ErrorState
      ...
      :message="rentalsStore.error"
      @retry="authStore.currentClientProfile && rentalsStore.fetchForClient(authStore.currentClientProfile.id)"
    />
