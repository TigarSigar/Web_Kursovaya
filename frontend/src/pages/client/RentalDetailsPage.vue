...
import { resolveImageUrls } from '@/utils/carousel'
...
const imageUrls = computed(() => resolveImageUrls(rental.value?.car))
...
const copy = computed(() =>
  locale.value === 'ru'
    ...
    : {
        ...
        confirmText:
          'Cancellation is allowed only before the rental reaches ISSUED. After issue, the action disappears from the interface.',
    ...
    <div v-else-if="rental && breakdown" class="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        ...
        <article class="card-base p-6">
          <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p class="mt-2 text-sm text-muted-foreground">
                {{ rental.car?.plateNumber }} • {{ rental.car?.location }}
              </p>
            </div>
            <StatusBadge :status="rental.status" />
          </div>

          <div class="mt-6 overflow-hidden rounded-lg border border-border/60 bg-surface/70">
            <img :src="selectedImage" alt="" class="h-72 w-full object-cover" />
          </div>
          <div v-if="imageUrls.length > 1" class="mt-4">
            <p class="text-sm text-muted-foreground">{{ copy.photos }}</p>
            <div class="mt-3 grid grid-cols-2 gap-3 md:grid-cols-4">
              <button
                v-for="(imageUrl, index) in imageUrls"
                :key="`${imageUrl}-${index}`"
                class="overflow-hidden rounded-lg border transition"
                :class="selectedImageIndex === index ? 'border-primary' : 'border-border/60'"
                type="button"
          ...
          <div class="mt-6 grid gap-4 md:grid-cols-2">
            <div class="rounded-2xl bg-surface/70 p-4">
              <p class="text-sm text-muted-foreground">{{ copy.period }}</p>
              <p class="mt-1 font-medium text-foreground">
                {{ formatDate(rental.from) }} - {{ formatDate(rental.to) }}
              </p>
            </div>
            <div class="rounded-2xl bg-surface/70 p-4">
              <p class="text-sm text-muted-foreground">{{ copy.actualReturn }}</p>
              <p class="mt-1 font-medium text-foreground">
                {{ rental.actualReturnAt ? formatDateTime(rental.actualReturnAt) : copy.notRecorded }}
              </p>
            </div>
            <div class="rounded-2xl bg-surface/70 p-4">
              <p class="text-sm text-muted-foreground">{{ copy.pickup }}</p>
              <p class="mt-1 font-medium text-foreground">{{ rental.pickupLocation }}</p>
            </div>
            <div class="rounded-2xl bg-surface/70 p-4">
              <p class="text-sm text-muted-foreground">{{ copy.return }}</p>
              <p class="mt-1 font-medium text-foreground">{{ rental.returnLocation }}</p>
            </div>
          </div>
        </article>
      ...
      <div class="space-y-6">
        ...
        <article class="card-base p-6">
          ...
          <p class="mt-4 font-medium text-foreground">{{ rental.client?.firstName }} {{ rental.client?.lastName }}</p>
          <p class="mt-2 text-sm text-muted-foreground">{{ rental.client?.email }}</p>
          <p class="mt-1 text-sm text-muted-foreground">{{ rental.client?.phone }}</p>
          <p class="mt-4 text-sm leading-7 text-muted-foreground">{{ copy.note }}</p>
        </article>

        <div class="flex gap-3">
          <button class="btn-secondary flex-1" type="button" @click="router.push('/client/rentals')">
            {{ copy.back }}
          </button>
          <button
            v-if="canCancel"
            class="btn-primary flex-1 bg-danger shadow-none hover:scale-100"
            type="button"
