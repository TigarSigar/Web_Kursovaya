...
const copy = computed(() =>
  locale.value === 'ru'
    ...
    : {
        ...
        title3: 'and Comfort',
        subtitle:
          'Search verified vehicles, see availability rules instantly, and book with transparent pricing built around real rental operations.',
        fleet: 'Fleet size',
...
<template>
  <section
    class="relative overflow-hidden rounded-2xl border border-border/50 bg-surface/85 px-5 py-8 shadow-panel lg:px-8 lg:py-10"
  >
    <div class="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(var(--color-accent),0.16),transparent_34%)]" />
    ...
    <div class="relative grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
      <div>
        ...
        <h1
          v-if="locale === 'ru'"
          class="mt-4 max-w-4xl text-4xl font-semibold tracking-tight text-foreground lg:text-6xl"
        >
          Путешествуйте <span class="text-primary">уверенно</span><br />
          Выбирайте <span class="text-primary">комфорт</span>.<br />
          Приезжайте стильно
        </h1>
        <h1 v-else class="mt-4 max-w-4xl text-4xl font-semibold tracking-tight text-foreground lg:text-6xl">
        ...
        <div class="glass-panel rounded-2xl p-4">
          <div class="grid gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-surface/70 p-3">
              <p class="text-sm text-muted-foreground">{{ copy.fleet }}</p>
              <p class="mt-1.5 text-2xl font-semibold text-foreground">{{ carsStore.items.length }}</p>
            </div>
            <div class="rounded-xl bg-surface/70 p-3">
              <p class="text-sm text-muted-foreground">{{ copy.available }}</p>
              <p class="mt-1.5 text-2xl font-semibold text-foreground">
                {{ carsStore.items.filter((car) => car.status === 'AVAILABLE').length }}
              </p>
            </div>
          </div>
        </div>
    ...
    <div class="relative mt-8">
      <SearchForm :locations="locations" @submit="submitSearch" />
    </div>
  </section>
  ...
  <section class="mt-8 grid gap-3 md:grid-cols-3">
    <div class="card-base p-4">
      ...
      <h2 class="mt-3 text-base font-semibold text-foreground">{{ copy.c1 }}</h2>
      <p class="mt-2 text-sm leading-6 text-muted-foreground">{{ copy.c1t }}</p>
    </div>
    <div class="card-base p-4">
      <Wrench class="h-6 w-6 text-primary" />
      <h2 class="mt-3 text-base font-semibold text-foreground">{{ copy.c2 }}</h2>
      <p class="mt-2 text-sm leading-6 text-muted-foreground">{{ copy.c2t }}</p>
    </div>
    <div class="card-base p-4">
      <ShieldCheck class="h-6 w-6 text-primary" />
      <h2 class="mt-3 text-base font-semibold text-foreground">{{ copy.c3 }}</h2>
      <p class="mt-2 text-sm leading-6 text-muted-foreground">{{ copy.c3t }}</p>
    </div>
  </section>
  ...
  <section class="mt-8">
    <div class="mb-4 flex items-end justify-between gap-4">
      <div>
        ...
        <h2 class="mt-2 text-2xl font-semibold text-foreground">{{ copy.sectionTitle }}</h2>
      </div>
