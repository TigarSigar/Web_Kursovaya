    ...
    <div v-else-if="grouped.length" class="grid gap-6 xl:grid-cols-2">
      <article v-for="tariff in grouped" :key="tariff.id" class="card-base p-6">
        <div class="flex items-start justify-between gap-4">
          <div>
            <p class="text-sm font-semibold uppercase tracking-[0.28em] text-primary">{{ humanizeEnum(tariff.carClass) }}</p>
        ...
        <div class="mt-4 grid gap-3 md:grid-cols-2">
          <div class="rounded-2xl bg-white/[0.03] p-4 text-sm text-muted-foreground">{{ locale === 'ru' ? 'База' : 'Base' }}: <span class="font-medium text-foreground">{{ formatCurrency(tariff.basePrice) }}</span></div>
          <div class="rounded-2xl bg-white/[0.03] p-4 text-sm text-muted-foreground">{{ locale === 'ru' ? 'Депозит' : 'Deposit' }}: <span class="font-medium text-foreground">{{ formatCurrency(tariff.depositAmount) }}</span></div>
          <div class="rounded-2xl bg-white/[0.03] p-4 text-sm text-muted-foreground">{{ locale === 'ru' ? 'Мин. срок' : 'Minimum' }}: <span class="font-medium text-foreground">{{ tariff.minimumDays }} {{ locale === 'ru' ? 'сут.' : 'days' }}</span></div>
          <div class="rounded-2xl bg-white/[0.03] p-4 text-sm text-muted-foreground">{{ locale === 'ru' ? 'Лимит' : 'Limit' }}: <span class="font-medium text-foreground">{{ tariff.mileageLimitKm }} km</span></div>
        </div>
