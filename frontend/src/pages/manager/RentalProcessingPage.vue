  ...
  <section>
    ...
    <div v-else-if="rental && breakdown" class="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p class="text-sm font-semibold uppercase tracking-[0.28em] text-primary">
                {{ locale === 'ru' ? 'Заказ' : 'Order' }} {{ rental.id }}
              </p>
              <h2 class="mt-2 text-2xl font-semibold text-foreground">
                {{ rental.car?.make }} {{ rental.car?.model }}
              </h2>
              <p class="mt-2 text-sm text-muted-foreground">
                {{ rental.client?.firstName }} {{ rental.client?.lastName }} • {{ rental.client?.email }}
              </p>
            </div>
          ...
          <div class="mt-6 grid gap-4 md:grid-cols-2">
            <div class="rounded-2xl bg-surface/70 p-4 text-sm text-muted-foreground">
              ...
              <span class="font-medium text-foreground"
                >{{ formatDate(rental.from) }} - {{ formatDate(rental.to) }}</span
              >
            </div>
            <div class="rounded-2xl bg-surface/70 p-4 text-sm text-muted-foreground">
              {{ copy.tariff }}: <span class="font-medium text-foreground">{{ rental.tariff?.name }}</span>
            </div>
            <div class="rounded-2xl bg-surface/70 p-4 text-sm text-muted-foreground">
              {{ copy.pickup }}: <span class="font-medium text-foreground">{{ rental.pickupLocation }}</span>
            </div>
            <div class="rounded-2xl bg-surface/70 p-4 text-sm text-muted-foreground">
              {{ copy.return }}: <span class="font-medium text-foreground">{{ rental.returnLocation }}</span>
            </div>
          </div>
      ...
      <div class="space-y-6">
        ...
        <button class="btn-secondary w-full justify-center" type="button" @click="router.push('/manager/rentals')">
          {{ copy.back }}
        </button>
      </div>
    </div>
