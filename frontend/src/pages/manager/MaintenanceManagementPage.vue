...
const copy = computed(() =>
  locale.value === 'ru'
    ? {
        ...
        emptyText:
          'Добавьте график сервисных работ, чтобы клиентский поиск корректно учитывал недоступность автомобилей.',
        create: 'Создать окно',
  ...
  <section>
    ...
    <div class="card-base p-5">
      <div class="flex flex-wrap gap-2">
        <button class="chip" :class="{ 'chip-active': filter === 'ALL' }" type="button" @click="filter = 'ALL'">
          {{ copy.all }}
        </button>
        <button
          class="chip"
          :class="{ 'chip-active': filter === 'SCHEDULED' }"
          type="button"
          @click="filter = 'SCHEDULED'"
        >
          {{ humanizeEnum('SCHEDULED') }}
        </button>
        <button
          class="chip"
          :class="{ 'chip-active': filter === 'IN_PROGRESS' }"
          type="button"
          @click="filter = 'IN_PROGRESS'"
        >
          {{ humanizeEnum('IN_PROGRESS') }}
        </button>
        <button
          class="chip"
    ...
    <ErrorState
      ...
      class="mt-6"
      :message="maintenanceStore.error"
      @retry="maintenanceStore.fetchAll"
    />
      ...
      <article v-for="item in filteredItems" :key="item.id" class="card-base p-5">
        ...
        <div class="mt-4 grid gap-4 md:grid-cols-3">
          <div class="rounded-2xl bg-surface/70 p-4 text-sm text-muted-foreground">
            ...
            <span class="font-medium text-foreground">{{ formatDate(item.from) }} - {{ formatDate(item.to) }}</span>
          </div>
          <div class="rounded-2xl bg-surface/70 p-4 text-sm text-muted-foreground">
            {{ locale === 'ru' ? 'Стоимость' : 'Cost' }}:
            <span class="font-medium text-foreground">{{
              item.estimatedCost ? formatCurrency(item.estimatedCost) : locale === 'ru' ? 'не указана' : 'not specified'
            }}</span>
          </div>
          <div class="rounded-2xl bg-surface/70 p-4 text-sm text-muted-foreground">
            {{ locale === 'ru' ? 'Локация' : 'Location' }}:
            <span class="font-medium text-foreground">{{ item.car?.location }}</span>
          </div>
        </div>
        ...
        <div class="mt-5 flex gap-3">
          <button
            ...
            type="button"
            @click="router.push(`/manager/maintenance/${item.id}/edit`)"
          >
            {{ t('common.edit') }}
          </button>
          <button class="btn-secondary flex-1 text-danger" type="button" @click="itemToRemove = item.id">
            {{ t('common.remove') }}
          </button>
        </div>
      </article>
