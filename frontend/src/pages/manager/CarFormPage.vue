...
const availableMakes = computed(() =>
  [...new Set(carsStore.items.map((car) => car.make).filter(Boolean))].sort((left, right) => left.localeCompare(right)),
)
...
const availableLocations = computed(() =>
  [...new Set(carsStore.items.map((car) => car.location).filter(Boolean))].sort((left, right) =>
  ...
  <section>
    ...
    <div v-else class="card-base p-6">
      ...
      <div class="mt-6 flex gap-3">
        <button class="btn-secondary" type="button" @click="router.push('/manager/cars')">
