...
class ClientProfileController(
    ...
    @GetMapping("/{id}")
    fun getClientById(@PathVariable id: Long): ClientResponse = clientProfileService.findById(id)

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    fun createClient(@Valid @RequestBody request: ClientCreateRequest): ClientResponse =
        clientProfileService.create(request)
