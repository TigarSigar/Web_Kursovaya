...
class RentalController(
    ...
    fun createRental(
        @Valid @RequestBody request: RentalCreateRequest,
    ): RentalResponse = rentalService.create(request)

    @PatchMapping("/{id}/issue")
    fun issueRental(
        @PathVariable id: Long,
    ): RentalResponse = rentalService.issue(id)

    @PatchMapping("/{id}/complete")
    fun completeRental(
        @PathVariable id: Long,
    ): RentalResponse = rentalService.complete(id)

    @PatchMapping("/{id}/cancel")
    fun cancelRental(
        @PathVariable id: Long,
    ): RentalResponse = rentalService.cancel(id)

    @GetMapping("/{id}")
    fun getRentalById(
        @PathVariable id: Long,
    ): RentalResponse = rentalService.findById(id)
}
